...
def create_attention_concat_point(window_size):
    ...
    output_layer = tf.keras.layers.Dense(1, activation='linear', name='output')(final_layer)
...
def create_attention(window_size):
    ...
    classification_output = tf.keras.layers.Dense(units=window_size, activation='sigmoid',
                                                  name="classification_output_temp")(x)
    classification_output_point = tf.keras.layers.Dense(units=1, activation='sigmoid',
    ...
    full_model = tf.keras.Model(inputs=input_data, outputs=[output, classification_output_point], name="LDwA")
