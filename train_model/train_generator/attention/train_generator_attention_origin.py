...
class TrainSlidingWindowGeneratorAttentionPure:

    def __init__(self, appliance_threshold, predict_mode, appliance_count, file_name, raw_file_name, shuffle, window_size, batch_size):
    ...
    def load_dataset(self):
        ...
        while True:
            for start_index in range(0, self.maximum_batch_size, self.__batch_size):
                ...
                output_data = np.array([outputs[index: index + self.__window_size] for index in splice])
                output_data = np.reshape(output_data, (output_data.shape[0], output_data.shape[1]))
                outputs_class = np.array([outputs_classification[index: index + self.__window_size] for index in splice])
                outputs_class = np.reshape(outputs_class, (outputs_class.shape[0], outputs_class.shape[1]))
