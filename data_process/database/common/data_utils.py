...
def single_normalization(df_align, meter_name, appliance_name, mode):
    ...
    section_name = meter_name + '_aggregate_' + appliance_name
    ...
    section_name_app = meter_name + '_' + appliance_name
    if cf.has_section(section_name_app):
        cf.remove_section(section_name_app)
    cf.add_section(section_name_app)
    cf.set(section_name_app, 'min', str(appliance_min))
    cf.set(section_name_app, 'max', str(appliance_max))
    if mode == 'file':
...
def single_normalization_test(df_align, meter_name, appliance_name):
    ...
    section_name_app = meter_name + '_' + appliance_name
    appliance_min = cf.getfloat(section_name_app, 'min')
    appliance_max = cf.getfloat(section_name_app, 'max')
    df_align['aggregate'] = standardize_data(df_align['aggregate'], aggregate_mean, aggregate_std)
    df_align[appliance_name] = normalize_data(df_align[appliance_name], appliance_min, appliance_max)
    df_align = society_normalization(df_align)
    return df_align
