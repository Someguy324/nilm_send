...
class TestSlidingWindowGeneratorAttentionPoint(object):
    def __init__(self, appliance_threshold, number_of_windows, input_window_length, predict_mode, appliance_name_list, test_directory, raw_file_name, appliance_count, offset):
        ...
        self.max_number_of_windows = self.total_size - input_window_length + 1
        self.__appliance_count = appliance_count
        self.__offset = offset

    def load_dataset(self):
        inputs, outputs = self.generate_test_data_mine()
    ...
    def generate_test_data_mine(self):
        data_array = self.data_array
        inputs = data_array[:, 0:4]
        # inputs = data_array[:, 0]
        inputs = np.reshape(inputs, (-1, 4))
